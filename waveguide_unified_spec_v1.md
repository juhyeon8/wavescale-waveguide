...
> 차단 모드가 있으면 d 가 C/κ_max 로 눌리므로, λ 가 큰 쪽이 오히려 도선이 많아진다.

#### 단계 8 브라우저 계시기 — 이번 변경에서 미측정 (2026-09-05)

κ·d 가드와 도선 간격 자동 규칙(C = 0.60)을 넣은 뒤 계시기를 돌리지 못했다.
브라우저 자동화가 다루는 탭이 `document.visibilityState = 'hidden'` 상태였고
(`innerWidth` 1920 > `outerWidth` 1536 — 오프스크린 렌더링으로 추정), 그 상태에서는
`requestAnimationFrame` 이 throttle 되어 나온 숫자를 게이트로 쓸 수 없다고 판단해
측정을 보류했다. 우회해서 얻은 값을 기록하지 않는다.

**따라서 차단 무대 600 ms / 슬라이더 최악 2500 ms 기준의 통과 여부는 미확인이다.**
정확성 게이트(G0·G1·TH.G2/G3/G4)는 통과했으나 이는 UI 반응 속도와 다른 것이다.

참고 규모 — 이번 변경으로 도선 수가 늘어난 폭:

| 조건 | d (종전 → 신규) | 양 벽 도선 | MoM O(n³) 기준 |
|---|---|---|---|
| P1 기본 a=6.0cm λ=14.4cm | 7.920 → 3.976 | 78 → 152 | 약 7.4배 |
| 차단 무대 끝 a=4.8cm λ=15cm | 8.250 → 3.128 | 74 → 194 | 약 18.0배 |
| 슬라이더 최악 a=6.0cm λ=2.4cm | 1.320 → 1.320 | 456 → 456 | 1.0배 (불변) |

최악 조건은 차단 모드가 없어 κ_max 가 정의되지 않으므로 d 가 바뀌지 않는다.
절대 도선 수는 여전히 최악 조건(456개)이 가장 많지만, 증가율은 차단 무대 끝이 가장 크다.

측정이 되는 대로 이 문단을 실측값으로 대체한다.
