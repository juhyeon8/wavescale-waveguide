...
> ⚠ 비용을 O(n³) 로 예측하지 말 것. 74~194도선 실측에서 시간은 도선 수에 선형이었다
> (약 1.06 ms/도선). 격자 항이 지배한다 — 아래 계시기 실측 절을 볼 것.
> λ가 크면 도선이 적어 가볍다는 종전 서술은 κ·d 규칙 도입(2026-09-05) 이후 성립하지 않는다.
> 차단 모드가 있으면 d 가 C/κ_max 로 눌리고, κ_max 는 λ 가 크거나 a 가 작을수록 커진다.

#### 단계 8 브라우저 계시기 — 실측 (2026-09-06) · 통과

κ·d 가드와 도선 간격 자동 규칙(C = 0.60) 적용 후 사이드바 [측정 (5회 중앙값)] 경로로 쟀다.
측정 환경: DPR = 0.9375, outer/inner = 0.7500, viewport 2048×972, vis = visible.
워밍업 2회 후 5회 중앙값. P1 조건은 재현성 확인을 위해 두 번 측정했다.

| 조건 | | d | 도선 | 영상법 | 도선관 | 합계 |
|---|---|---|---|---|---|---|
| P1 기본 λ=14.4cm a=6.0cm | 변경 전 | 7.920 | 78 | 67 · 68 | 85 · 87 | **175 · 171** |
| | 변경 후 | 3.976 | 152 | 73 · 69 | 163 · 153 | **252 · 237** |
| 차단 무대 끝 λ=15.0cm a=4.8cm | 변경 전 | 8.250 | 74 | 54 | 81 | **147** |
| | 변경 후 | 3.128 | 194 | 64 | 202 | **276** |

**판정: 차단 무대 합계 최대 276 ms — 기준 600 ms 의 46 %로 통과.**

슬라이더 최악(λ=2.4cm)은 재지 않았다. d 가 변경 전후 1.320 으로 같아 이번 변경이 닿지
않는 조건이다. 아래 선형 관계로 **추정**하면 도선관 456 × 1.06 ≈ 482 ms, 합계 550 ms
안팎으로 기준 2500 ms 에 크게 못 미친다. 실측이 아니라 추정값이다.

##### 이번 측정의 핵심 발견 — 계산 시간은 도선 수에 선형이다

적용 전 이 절은 비용을 MoM 의 O(n³) 로 예측했다. 실측은 전혀 달랐다.

| 조건 | 도선 증가 | O(n³) 예측 | 실측 합계 | 실측 도선관 |
|---|---|---|---|---|
| P1 기본 | 78 → 152 (1.95배) | 7.4배 | **1.44배** | **1.87배** |
| 차단 무대 끝 | 74 → 194 (2.62배) | 18.0배 | **1.88배** | **2.49배** |

도선관 시간의 증가폭(1.87 · 2.49배)이 도선 수 증가폭(1.95 · 2.62배)과 거의 같다.
도선당 단가로 보면 1.115 → 1.072, 1.095 → 1.041 ms/도선으로 사실상 일정하다.
**연립방정식 풀이(O(n³))가 아니라 격자 위 장 계산(O(n × Nx × Ny))이 지배한다.**
격자가 520×220 = 114400 점이라 도선 하나를 늘릴 때마다 그만큼의 Hankel 평가가
붙는데, 이 항이 측정 범위에서 MoM 항을 압도한다(152도선에서 1.74×10⁷ 대 3.5×10⁶).

> ⚠ 456도선(λ=2.4cm)은 측정 범위(74~194도선) 밖이다. 그 지점에서는 MoM 9.5×10⁷ 이
> 격자 5.2×10⁷ 을 넘어서므로 선형 외삽 550 ms 는 하한으로 봐야 한다. 다만 두 항을
> 다 더해도 2500 ms 기준에는 크게 못 미친다.

##### 성능 프로파일이 뒤집혔다

변경 전에는 긴 파장 쪽(147 ms)이 P1 기본(175 ms)보다 가벼웠으나, 변경 후에는
276 ms 로 P1(252 ms)보다 무겁다. 차단 모드가 있으면 d 가 C/κ_max 로 눌리는데,
κ_max = √(k_c² − k²) 는 λ 가 크거나 a 가 작을수록 커진다. 차단 무대 끝은 두 조건이
겹친다(λ 14.4 → 15.0 cm, a 6.0 → 4.8 cm): κ₃ 가 0.1509 에서 0.1918 로 커지고
d 가 3.976 → 3.128 로 더 눌려 도선이 152 → 194 개가 된다.
따라서 "슬라이더 왼쪽 끝에서만 무겁다" 는 종전 전제는 더는 성립하지 않는다.

##### 대조군 — 변하지 않아야 할 것은 변하지 않았다

영상법은 67 · 68 → 73 · 69 ms, 렌더는 9~10 ms 로 변경 전후가 같다. 둘 다 도선 간격과
무관한 경로이므로 이것이 맞다. 늘어난 것은 도선관 열 하나뿐이다.
