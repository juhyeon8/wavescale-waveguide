(function (global, factory) {
  ...
  //
  // ⚠ 이 가드는 벽 이산화 가드(κ·d ≤ C)를 대신하지 못한다. 벽이 너무 성기면 오차가
  //   커지는데도 로그 진폭은 여전히 곧다: κ·d = 1.5 에서 오차 −6.35 / −4.63 / −3.85% 인
  //   세 조건의 R²가 0.9923 / 0.9961 / 0.9878 로 전부 이 문턱을 넘는다. 둘 다 필요하다.
  var R2_MIN = 0.96;
  ...
  function measureKappa(field, a, n, winId, d, kThy) {
    ...
    /* 벽 이산화 가드 — κ·d ≤ C.  (종전: 1/κ ≥ d, 즉 κ·d ≤ 1)
     * 형태는 종전이 옳았고 상수만 느슨했다. κ·d 가 지배 변수라는 것은 D-DSCAN 이 확인했다:
     * κ₃가 2.6배 다른 세 조건(T2 0.1405 / T3 0.0702 / T5 0.1817)의 오차 곡선이 κ·d 축에서
     * 겹치고, κ·d = 1.5 에서 셋이 동시에 무너진다(−6.35 / −4.63 / −3.85%). 그 지점의 d/λ는
     * 0.214 / 0.214 / 0.066 으로 3배 넘게 다르므로 축은 d/λ 가 아니다.
     * C = GEO.KAPPA_D_MAX 는 |오차| ≤ 0.5% 보장선에서 읽었다 (근거는 geometry.js 주석).
     * 종전 상수 1 은 이 선의 1.7배였고, 그래서 κ·d = 0.98 인 조건이 +5.1% 로 통과했다.
     *
     * ⚠ 이 가드를 R² 가드와 중복이라고 지우지 말 것. κ·d = 1.5 의 −6.4% 는 R² 0.9878~0.9961
     *   이라 R²_MIN(0.96)을 통과한다. 두 가드는 서로 다른 실패를 잡는다. */
    if (!(kThy * (d || 1) <= GEO.KAPPA_D_MAX))
      return { value: null, reason: '벽 이산화 한계 (κ·d=' + (kThy * (d || 1)).toFixed(3) +
                                    ' > ' + GEO.KAPPA_D_MAX + ')', win: win };
